...
#             query=req.query, user_id=req.user_id, max_docs=req.max_docs
#         )
#         return response
...
# Updated imports (consistent with your folder structure)
from src.agents.orchestrator import Orchestrator
from src.utils.ingestion.retriever import HybridRetriever
...
def rag_endpoint(req: Query):
    try:
        response = orchestrator.run(
            query=req.query,
            user_id=req.user_id,
            max_docs=req.max_docs or 5,   # fallback in case None
        )
        return response
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
